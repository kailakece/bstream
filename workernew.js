...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
        ...
        if (typeParam === "live" || typeParam === "film" || typeParam === "semi") {
          ...
          if (parentFound) {
            ...
            if (parts[0].toLowerCase() === requestedEps) {
              ...
              referer = parts[2] || "";
              userAgent = parts[3] || "";
              drmType = parts[4] || ""; // Menyesuaikan indeks kolom untuk tipe series
              drmLicense = parts[5] || "";
              break; 
            }
          }
      ...
      // Memasukkan data DRM ke fungsi generatePlayerHtml
...
function generatePlayerHtml(targetUrl, referer, userAgent, urlObj, drmType = "", drmLicense = "") {
  ...
  // 2. JIKA EXTERNAL EMBED / WEB IFRAME
  ...
  // 3. JIKA DETEKSI FORMAT MPD (DASH) ATAU MEMILIKI DRM -> GUNAKAN SHAKA PLAYER
  ...
  // 4. JIKA FORMAT HLS (.M3U8) ATAU NATIVE MP4 STANDAR (TANPA DRM)
  ...
  <body>
      ...
      <script>
          ...
          if ("${playerType}" === "hls" && typeof Hls !== 'undefined' && Hls.isSupported()) {
              ...
              hls.on(Hls.Events.ERROR, function(e, d) { if (d.fatal) playRandomFallback(); });
          } else {
